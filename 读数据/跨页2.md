...
{{forloop.index}}|{{a[0]-}}|{{b|newline_to_br|newline_to_br|replace:'|','&vert;'|replace:'<','&lt;'|truncate:200,c}}
{%endfor-%}
{%endunless-%}
{%capture b%}{{a}}{%endcapture-%}
{%capture c%}…（{{b|size}}）{%endcapture-%}
{{a|size}}||{{b|newline_to_br|newline_to_br|replace:'|','&vert;'|replace:'<','&lt;'|truncate:200,c}}
